...
const expoundList = [
    ...
    "主题的发生，到底需要如何做到，没有主题的发生，又会如何产生影响？",
    "而这些并不是完全重要，更加重要的问题是，",
    "主题，到底应该如何实现。",
    "带着这些问题，我们来审视一下主题。",
    "所谓主题，关键是主题需要如何写。",
...
const connectingBefore = [
    ...
    "但是，仍有几个思想表明这并不在所有情况下都成立。",
    "这似乎解答了我的疑惑。"
]

const connectingAfter = [
    "提到",
    "指出",
    "说过",
    "曾经说过",
...
function getWisdom() {
    let wisdom = initSentence(wisdomList)
    wisdom = wisdom.replace("曾经说过", initSentence(connectingAfter))
    wisdom = wisdom.replace("这不禁令我深思", initSentence(connectingBefore))
    return wisdom
}

function getExpound() {
    let topic = $("#topic").value
    let expound = initSentence(expoundList)
    expound = expound.replace(RegExp("主题", "g"), topic)
    return expound
}
...
function getEnding() {
    ...
    } else return getWisdom()
}

function createBullshit() {
    let article = []
    let articleLength = 0
    let section = `初次遇到${$("#topic").value}的问题时，我进行了大量的思考。`
    let setLength = $("#article-length").value
    let para = $("#para").value
    while (articleLength < setLength) {
        while (section.length < 800) {
            let p = randomNum()
            //段落字数超过140后结束段落概率随字数增加而增大，增大程度由para决定
            if (p < ((section.length - 140) / para)) break
            //来点好听的
            else if (p > 85) section += getWisdom()
            //来点废话
            else section += getExpound()
        }
        section += getEnding()
        article.push(section)
        articleLength += section.length
        section = ""
    }
    article[article.length - 1] += "以上就是我的全部观点。"
    $("#article").innerHTML = "<p>" + article.join("</p><p>") + "</p>"
    $("#count").innerHTML = `共 ${articleLength + 11} 个字`
}